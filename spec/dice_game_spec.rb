...
describe Dice do
  ...
  it "'Subject.previous_rolls' should should push previous dice rolls" do
    rolls = subject.roll(3)
    expect(subject.previous_rolls).to include(rolls)
 end

  it "Should return a total of previous rolls" do
    total = subject.roll(3).inject(:+)
    expect(subject.score).to eq (total)
  end
end